...
| Detect broken research structure | Read-only `check` with separate nonblocking research warnings, structured diagnostics, duplicate/ambiguous/missing links, source types, unsafe/missing attachments, malformed privacy flags. |
...
| Explore locally | Embedded browser graph, search, generation layout, branch selection, parent-role labels, local portrait thumbnails, occupations, stories, and note composition; relationship filters, details/evidence/events, keyboard navigation, safe attachment access. Maternal/paternal ordering and selection highlights retain relatives and connection paths. Native browser checks, pure graph tests, and loopback HTTP API tests. |
